...
//                    .subscribe(commentRealm -> productRealm.getComments().add(commentRealm)); //Добавляем в объек
//        }
    ...
    public void deleteFromRealm(Class<? extends RealmObject> objectClass, String id) {
        ...
        RealmObject entity = realm.where(objectClass).equalTo("id",id).findFirst(); //находим запись по идентификатору
